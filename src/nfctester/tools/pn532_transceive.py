...
def run_atqa_poll_test():
    ...
    """
    # --- 追踪配置 ---
    trace.filter.protocol = True     # 开启协议追踪
    trace.filter.driver = False      # 关闭底层驱动报文追踪
    trace.filter.level = "debug"     # 设置日志级别
    # ----------------
    ...
    try:
        ...
        # 建立连接
        reader.open()
        trace.debug("开始循环轮询 (REQA/WUPA) (按 Ctrl+C 退出)...")
        time.sleep(0.1)
        ...
        while True:
            ...
            try:
                ...
                if res.data:
                    trace.app(f"收到响应 (ATQA) [cmd 26]: {res.data.hex(' ').upper()}")
                    found = True
                    break
